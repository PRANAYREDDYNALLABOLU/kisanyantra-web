...
export default function ContactPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError('Could not send message. Please call us directly at +91 9182817019');
  ...
  const inp = (field, placeholder, type = 'text') => ({
    ...
    style: {
      ...
      border: '1.5px solid #E8E5DE', borderRadius: 12,
    ...
    <div style={{ background: '#FAFAF8', minHeight: '100vh', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '64px 24px 80px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 40, alignItems: 'start' }}>
          ...
          {/* Contact info + FAQ */}
          <div>
            <h2 style={{ fontSize: 24, fontWeight: 800, fontFamily: 'Fraunces, serif', marginBottom: 32, color: '#1A1A1A' }}>Contact details</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 40 }}>
              {[
                ...
                <a key={i} href={item.href} style={{
                  ...
                  border: '1px solid #E8E5DE', textDecoration: 'none', transition: 'all 0.2s',
                }}
                ...
                >
                  ...
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#40916C', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 3 }}>{item.title}</div>
            ...
            {/* FAQ */}
            <h3 style={{ fontSize: 18, fontWeight: 800, fontFamily: 'Fraunces, serif', marginBottom: 20, color: '#1A1A1A' }}>Common questions</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                { q: 'How do I book equipment?',              a: 'Download the KisanYantra app, enter your farm location, pick equipment and confirm. A driver will accept within minutes.' },
                ...
                <details key={i} style={{ background: '#fff', borderRadius: 14, border: '1px solid #E8E5DE', overflow: 'hidden' }}>
                  <summary style={{ padding: '14px 18px', fontWeight: 700, fontSize: 14, color: '#1A1A1A', cursor: 'pointer', listStyle: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    {faq.q}
                    <span style={{ color: '#40916C', fontSize: 18, fontWeight: 400, flexShrink: 0, marginLeft: 8 }}>+</span>
                  </summary>
                  <div style={{ padding: '0 18px 14px', color: '#6A6A6A', fontSize: 14, lineHeight: 1.7 }}>{faq.a}</div>
                </details>
              ))}
            </div>
          </div>
            ...
            {submitted ? (
              <div style={{ textAlign: 'center', padding: '40px 0' }}>
                ...
                <p style={{ color: '#6A6A6A', fontSize: 15, lineHeight: 1.7, marginBottom: 24 }}>
                  Thank you for reaching out. We will get back to you within 24 hours on your phone number.
                </p>
                <button
              ...
              <>
                ...
                {error && (
                  <div style={{ background: '#FFEBEE', border: '1px solid #FFCDD2', borderRadius: 10, padding: '12px 16px', marginBottom: 16, color: '#e53935', fontSize: 13, fontWeight: 600 }}>
                    ❌ {error}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div>
                    ...
                    <input {...inp('phone', '+91 9XXXXXXXXX', 'tel')} />
                  </div>
                  ...
                  {/* Type */}
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 700, color: '#3A5A3A', marginBottom: 8 }}>I am a...</label>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: '#fff', fontWeight: 800, padding: '15px',
                      borderRadius: 14, border: 'none', fontSize: 16,
                      cursor: loading ? 'not-allowed' : 'pointer',
                      transition: 'all 0.2s', marginTop: 4, width: '100%',
                    }}
                    onMouseEnter={e => { if (!loading) e.currentTarget.style.background = '#2D6A4F'; }}
                    onMouseLeave={e => { if (!loading) e.currentTarget.style.background = '#1B4332'; }}
                  >
